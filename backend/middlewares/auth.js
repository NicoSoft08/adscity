...
const checkAdminRole = async (req, res, next) => {
    try {
        ...
        res.status(401).json({ message: 'Utilisateur non authentifié ou rôle non autorisé.' });
    }
};
...
const checkUserRole = async (req, res, next) => {
    try {
        ...
        if (userData.role === 'user') {
            ...
            res.status(403).json({ message: 'Accès refusé : Vous n\'êtes pas utilisateur.' });
        }
    } catch (error) {
...
module.exports = { checkAuth, checkAdminRole, checkUserRole };